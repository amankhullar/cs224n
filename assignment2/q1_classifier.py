...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        config = Config()
        W = tf.get_variable(name = 'weights', shape = (config.n_features, config.n_classes), initializer = tf.zeros_initializer)
        b = tf.get_variable(name = 'bias', shape = (1, config.n_classes), initializer = tf.zeros_initializer)
