...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        preds, state = tf.nn.dynamic_rnn(cell = cell, inputs = x, dtype = tf.float32)
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients is True:
            clipped_gradients = [(tf.clip_by_global_norm(gvs[0], clip_norm = self.config.max_grad_norm), gvs[1]) for gvs in gradients_variables]
            gradients_variables = clipped_gradients
        gradients = [gvs[0] for gvs in gradients_variables]
