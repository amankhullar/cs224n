...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        sentence_temp = copy.copy(sentence)
        sentence_length = len(sentence)
        count = 0
        for i in range(window_size):
            sentence_temp.insert(0,start)
            count += 1
            sentence_temp.append(end)
        for i in range(count, count + sentence_length):
            temp_window = []
            for j in range(window_size,0,-1):
               temp_window += sentence_temp[i-j]
            for j in range(window_size + 1):
                temp_window += sentence_temp[i+j]
            windowed_data.append((temp_window, labels[i - count]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding = tf.get_variable(name = "embedding", initializer = self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name = "Weights1", shape = [self.config.n_window_features * self.config.embed_size, self.config.hidden_size], initializer = tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(name = "bias1", shape = [self.config.hidden_size, ], initializer = tf.contrib.layers.xavier_initializer())
        U = tf.get_variable(name = "Weights2", shape = [self.config.hidden_size, self.config.n_classes], initializer = tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable(name = "bias2", shape = [self.config.n_classes, ], initializer = tf.contrib.layers.xavier_initializer())
