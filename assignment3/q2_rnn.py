...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if original_length > max_length:
            del sentence[max_length:]
            del labels[max_length:]
            mask_vector = [True] * max_length
            ret.append((sentence, labels, mask_vector))
        else:
            sentence_new = sentence + [zero_vector] * (max_length - original_length)
            labels_new = labels + [zero_label] * (max_length - original_length)
            mask_vector = [True] * original_length + [False] * (max_length - original_length)
            ret.append((sentence_new, labels_new, mask_vector))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(dtype = tf.int32, shape = (None, self.max_length, Config.n_features), name = "input")
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable(name = 'b2', initializer = tf.zeros(shape = [Config.n_classes, ]))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                output_at_time, new_state = cell(x[:, time_step, :], state, scope = "RNN")
                output_drop = tf.nn.dropout(output_at_time, dropout_rate)
                preds.append(tf.matmul(output_drop, U) + b2)
                ### END YOUR CODE
